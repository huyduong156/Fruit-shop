                ...
                <div class="row flex-row-reverse">
                    <div class="col-lg-9 col-12 col-custom widget-mt">
                        ...
                        <div class="row shop_wrapper grid_3">
                            {{-- @if ($product_cat->contains(null))
                                <h4>Chưa có sản phẩm nào trong danh mục này</h4>
                            @endif --}}
                            @foreach ($product_tag as $prod)
                            <div class="col-md-6 col-sm-6 col-lg-4 col-custom product-area">
                                <div class="single-product position-relative">
                                    <div class="product-image">
                                        <a class="d-block" href="{{route('home.product',$prod->slug)}}">
                                            <img src="uploads/product/{{$prod->image}}" alt="" class="product-image-1 w-100">
                                            @if ($prod->images->first())
                                                <img src="uploads/product/{{$prod->images->first()->image}}" alt="" class="product-image-2 position-absolute w-100">
                                            @endif
                                        </a>
                                    </div>
                                    <div class="product-content">
                                        <div class="product-rating">
                                            <div class="rateit" data-rateit-value="{{$prod->AverageRating()}}" data-rateit-ispreset="true" data-rateit-readonly="true"></div>
                                        </div>
                                        <div class="product-title">
                                            <h4 class="title-2"> <a href="{{route('home.product',$prod->slug)}}">{{$prod->name}}</a></h4>
                                        </div>
                                        <div class="price-box">
                                    ...
                                    <div class="product-content-listview">
                                        ...
                                        <div class="product-title">
                                            <h4 class="title-2"> <a href="product-details.html">{{$prod->name}}</a></h4>
                                        </div>
                                        <div class="price-box">
                                            @if ($prod->sale_price > 0)
                                                <span class="regular-price ">{{number_format($prod->sale_price)}}VND</span>
                        ...
                        @if(isset($product_tag) && count($product_tag) > 0 )
                            {{$product_tag->links('home.blocks.pagination_template')}}
                        @endif
                        <!-- Bottom Toolbar End -->
                    </div>
